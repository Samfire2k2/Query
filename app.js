...
const requiredEnvVars = ['DISCORD_TOKEN', 'PUBLIC_KEY', 'APP_ID', 'HUGGINGFACE_API_KEY'];
  ...
  if (body.type === 2) {
    ...
    if (commandName === 'summarize') {
      try {
        ...
        // Fetch messages from channel (limited to last 100 for Hugging Face free tier)
    ...
    if (commandName === 'search_summarize') {
      try {
        ...
        await sendFollowup(`⏳ Fetching last 100 messages and searching for "${keyword}"...`, body.token);
        const messages = await fetchChannelMessages(channelId, process.env.DISCORD_TOKEN);
